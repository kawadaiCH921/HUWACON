...
class MainScene extends Phaser.Scene {
  ...
  collectAssets(stageData) {
    ...
    assetList.add({ key: stageData.stage.background.image, path: `img/1-4/${stageData.stage.background.image}.png` }); // 変更要
  ...
  create() {
   ...
   // 敵キャラの生成 (動的グループ)
    ...
    this.cameras.main.setZoom(1); // カメラのズームを設定（アップさせる）
...
shootBullet(enemy, bulletSpeed, bulletImage) {
  ...
  onGoalReached() {
    ...
    gameClearSound.once('complete', () => {
      ...
      const url = `clear.php?stage_id=4&time=${encodeURIComponent(finalTime.toFixed(3))}`;
